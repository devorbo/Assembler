...
int addressing_method(char *word,int *address_type,int there_is_label,int op,symbol_type ***symbol_table,int *num_symbol,line_info curr_line_info)
{
    ...
    if(word[0]=='#')/* immediate */
      ...
      else if(word[0]=='%')/* relative */
      {
          if(op==9)
          {
              if(symbol_is_extern(word+1,symbol_table,num_symbol)!=ERROR)/* label is extern or not exist */
              {
                  fprintf(stdout,"Error in file %s line %d: Label of relative type could not come from an external file\n", curr_line_info.file_name, curr_line_info.cnt_line);
                  return ERROR;
              }
